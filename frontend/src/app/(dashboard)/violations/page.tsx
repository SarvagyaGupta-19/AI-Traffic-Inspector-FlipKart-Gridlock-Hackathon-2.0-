...
import { Search, Filter, Trash2, ExternalLink, ChevronLeft, ChevronRight, X, AlertTriangle, CheckCircle2 } from 'lucide-react';
...
export default function ViolationsPage() {
  ...
  const handleStatusUpdate = async (id: number, status: 'issued' | 'rejected') => {
    try {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-3">
              <span className="text-[var(--accent-blue)]">ASTraM Evidence Portal</span>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--border-color)]">
                    ...
                    <th className="text-right text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider px-5 py-4">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {violations.map((v) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <span className="badge badge-purple text-[10px] opacity-70">Pending Review</span>
                      </td>
                      <td className="px-5 py-4">
                        {v.evidence_url ? (
                          <a
                      ...
                      <td className="px-5 py-4 text-right">
                        <button
                          ...
                          className="p-2 rounded hover:bg-[#FFEBE6] text-[var(--text-secondary)] hover:text-[var(--accent-red)] transition-colors"
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setSelectedViolation(null)}>
          <div className="bg-white rounded shadow-lg border border-[var(--border-color)] w-full max-w-2xl mx-4 max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-4 border-t border-[var(--border-color)] bg-[#F8F9FA] flex gap-3 justify-end">
              <button
                onClick={() => handleStatusUpdate(selectedViolation.id, 'rejected')}
                className="btn-secondary text-[var(--accent-red)] flex items-center gap-2"
              >
                <X className="w-4 h-4" />
                Dismiss (False Positive)
              </button>
              <button
                onClick={() => handleStatusUpdate(selectedViolation.id, 'issued')}
